...
LinkedBlockingQueue* constructLinkedBlockingQueue(void *addr)
{
	...
	if(err)
	{
		fprintf(stderr, "pthread_mutexattr_init error. [err=%s]", strerror(err));
		return NULL;
	}
	...
	if(err)
	{
		fprintf(stderr, "pthread_mutexattr_settype error. [err=%s]", strerror(err));
		return NULL;
	}
	...
	if(err)
	{
		fprintf(stderr, "pthread_mutex_init error. [err=%s]", strerror(err));
		return NULL;
	}
	...
	if(err)
	{
		fprintf(stderr, "pthread_cond_init error. [err=%s]", strerror(err));
		return NULL;
	}
...
void destructLinkedBlockingQueue(LinkedBlockingQueue *linkedBlockingQueue)
{
	...
	if(err)
	{
		fprintf(stderr, "pthread_cond_destroy error. [err=%s]", strerror(err));
	}

	err = pthread_mutex_destroy(&linkedBlockingQueue->mutex);
	if(err)
	{
		fprintf(stderr, "pthread_mutex_destroy error. [err=%s]", strerror(err));
	}

	LinkedQueue *linkedQueue = container_of(linkedBlockingQueue->queue, LinkedQueue, iqueue);
	delete(LinkedQueue, linkedQueue);
}

void LinkedBlockingQueue_push(IQueue *iqueue, ICommand *command)
{
	LinkedBlockingQueue *linkedBlockingQueue = container_of(iqueue, LinkedBlockingQueue, iqueue);

	int err = pthread_mutex_lock(&linkedBlockingQueue->mutex);
	if(err)
	{
		fprintf(stderr, "pthread_mutex_lock error. [err=%s]", strerror(err));
		return;
	}
	...
	if(err)
	{
		fprintf(stderr, "pthread_mutex_unlock error. [err=%s]", strerror(err));
	}

	err = pthread_cond_signal(&linkedBlockingQueue->cond);
	if(err)
	{
		fprintf(stderr, "pthread_cond_signal error. [err=%s]", strerror(err));
	}
}
...
ICommand* LinkedBlockingQueue_pop(IQueue *iqueue)
{
	...
	{
		err = pthread_cond_wait(&linkedBlockingQueue->cond, &linkedBlockingQueue->mutex);
		if(err)
		{
			fprintf(stderr, "pthread_cond_wait error. [err=%s]", strerror(err));
			return NULL;
		}
	}

	ICommand *command = linkedBlockingQueue->queue->pop(linkedBlockingQueue->queue);
	err = pthread_mutex_unlock(&linkedBlockingQueue->mutex);
	if(err)
	{
		fprintf(stderr, "pthread_mutex_unlock error. [err=%s]", strerror(err));
